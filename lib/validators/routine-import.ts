...
export const routineImportRowSchema = z.object({
  ...
  repsScheme: z.preprocess(emptyToUndefined, z.string().trim().max(100).optional()),
  weightKg: z.preprocess(emptyToUndefined, z.coerce.number().positive().optional()),
  intensity: z.preprocess(emptyToUndefined, z.string().trim().max(20).optional()),
  tempo: z.preprocess(emptyToUndefined, z.string().trim().max(20).optional()),
  ...
  groupLabel: z.preprocess(emptyToUndefined, z.string().trim().max(10).optional()),
  groupRestSecs: z.preprocess(emptyToUndefined, z.coerce.number().int().nonnegative().optional()),
