...
export default async function CoachStudentRoutinePage({
  params,
}: {
  params: Promise<{ coachSlug: string; dni: string }>
}) {
  const { coachSlug, dni: rawDni } = await params
  ...
  if (!parsed.success) redirect(`/${coachSlug}?error=invalid`)

  const student = await studentService.getByDni(parsed.data, coach.id)
  if (!student) redirect(`/${coachSlug}?error=not-found`)
